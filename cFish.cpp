...
#include "cBullet.h"
#include <string>
cFish::cFish(const D3DXVECTOR2& pos, const D3DXVECTOR2& playerPos, const float speed)
	: cEnemy(pos, 300, 2), m_playerpos(playerPos), m_PlayerBullet(GAMEMANAGER->getPBulletAd()->GetPBullet())
{
	m_image = IMAGEMANAGER->FindImage("IdleFish_0");
	m_speed = speed;
	m_EnemyAni[eIDLE] = new cAnimation("IdleFish", 0.08f, true); 
	m_EnemyAni[eDAMAGE] = new cAnimation("DamageFish", 0.08f, true); 
	m_EnemyAni[eDIE] = new cAnimation("DieFish", 0.08f, false);
	m_State = eIDLE;
